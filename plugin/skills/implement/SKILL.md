...
6. Push and open the PR with `gh pr create --title "<conventional title>" --body-file <file> --milestone "<the issue's milestone>"`, adding `--reviewer <name>` for each entry in `.visionary.yml` `reviewers` so a person is asked to review every PR an agent opens. If the flag fails because a name cannot be requested, say so in the PR body rather than failing the run. The body follows the conventions (marker `<!-- visionary:implement issue=$issue -->`, then the implementer byline) and includes `Closes #$issue`. Comment on the issue with the PR link.
