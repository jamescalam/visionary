...
### Changed
- The bench workflow publishes what the suite writes as a downloadable artifact and links it from the bench comment (`publish`), and caches a run-history file between runs (`history`) so trend charts have data on a fresh runner.
