...
### Changed
- A single inline comment on the diff now triggers the responder. It wraps itself in a review with an empty body, so the `pull_request_review` trigger alone missed it.
