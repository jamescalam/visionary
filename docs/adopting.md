...
A comment the responder picks up gets an eyes reaction within seconds, before any work starts, so you know it was seen. A submitted review is acknowledged on its inline comments; a review with only a summary has no reaction target in GitHub’s API and gets a short "seen, working on it" comment instead, deleted when the run ends so it never sits beside the reply. Its reply comes later in the same thread. If the run fails it says so in a comment rather than going quiet. On a pull request the running check is also visible in the checks list.

The responder runs on every human comment on a PR or issue an agent opened, and on any comment mentioning `//vision` elsewhere. A single inline comment on the diff arrives as a submitted review with an empty body, so the decision of whether a comment is for the role is made by a gate job that reads the review's inline comments, not by the workflow's `if`. Leave a comment anywhere and it is seen. On a proposal issue, a comment can ask the responder to revise the proposal body; it never adds `approved`. The phrase is `//vision`, deliberately not an @-mention: GitHub links every `@name` in a comment to a real user, so an invented handle would notify a stranger, and `@claude` collides with the Claude GitHub App's own mention handling. Change it in the caller's `if:` if you want another.
