...
A comment the responder picks up gets an eyes reaction within seconds, before any work starts, so you know it was seen. Its reply comes later in the same thread. If the run fails it says so in a comment rather than going quiet. On a pull request the running check is also visible in the checks list.
